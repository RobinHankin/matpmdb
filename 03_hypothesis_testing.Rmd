...
The standard deviation of the sample mean is known as tthe *standard
...
null by calculating the p-value.  We measure consistency by calculating the p-value:
...
```{r}
t.test(before,after, var.equal=TRUE, paired=TRUE,alternative="less")
```
...
```{r}
x <- 0:100
plot(x,dbinom(x,100,1/2),col=c(rep("black",59),rep("red",42)),type="h",lwd=2,main="null distribution; observation=58")
legend("topleft",lty=1,lwd=2,col=c("red","black"),legend=c("more extreme","distribution"))
```
...
such intervals are shown in red.  See how much of the R idiom you can
understand.
