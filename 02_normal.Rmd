...
larger values of n.  The following few figures show table plots of
...
If we want to find the probability that $X$ lies between two values
$a$ and $b$, then this would be represented as the compound statement
$(X>a)\quad\&\quad(X<b)$ as there are two conditions present^[for
simplicity I am ignoring the difference between "$<$" and "$\leq$"].
This is more usually written $a<X<b$.  We can visualise this easily.
Suppose we have a standard Gaussian $N(0,1)$ and want the probability
that $-0.4<X<1.1$:
...
In the above, we had ```1e6``` (that is, $10^6$ or one million)
samples, of which 96312 were over 1.3.  This corresponds to a
probability of 96312/1000000, or a little over 0.096


We can make a different visualization of the same situation as follows:
...
In the above I have divided the table results by the number of obsevations so the numbers represent probabilities.
...
The mean of a distribution [sometimes "population mean"] has a
technical meaning but here we can say that it is defined as the
long-run average of observations drawn from that distribution.  The
mean of the Gaussian distribution is~$\mu$, and this is easy to
demonstrate in R:
...
In the above, we have one hundred samples from a $N(10,2)$
distribution, and the average of these is roughly 10 (but not exactly,
due to the finite sample size).  Observe carefully that we cannot tell
...
median, as it is symmetric and unimodal.

Note that some distributions do not have a mean (for example, the
Cauchy distribution).

### Variance and standard deviation of a distribution

The standard deviation measures the "width" of a distribution, again
with a specific technical meaning.  In R, we can estimate the standard
deviation of a sample using the ```sd()``` command:
...
$np(1-p)=6\times\frac{1}{3}\times\frac{2}{3}=\frac{4}{3}$.  Verifying
this numerically is straightforward:
...
We will see the same technique used for other distributions later in
the course.
