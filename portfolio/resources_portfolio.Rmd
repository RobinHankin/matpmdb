...
The first line shows that the quincunx (a mechanical illustration of the Binomial distribution) was acquired in 2012 at zero cost (zero cost is represented as one cent), and it has been used 9 times.  There are 20 resources in total; the figure below shows some of them.  
...
the (two-sided) pvalue is not significant although the one-sided value is.  We might wonder whether the expensive items are more heavily used.  For this, we use Fisher's exact test with "expensive" meaning costing more than one cent, and "heavily used" meaning "used more than 7 times":
