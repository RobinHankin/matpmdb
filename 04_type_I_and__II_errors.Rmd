...
standard critical region is to reject $H_0$ if our observation exceeds
a particular critical value.
...
For simplicity we will consider only one-sided tests.

If we have observations from $N(\mu,1)$ [that is, Gaussian with
unknown mean $\mu$ and standard deviation 1], we may wish to test
$H_0\colon\mu=0$.
...
The type I error has an inverse.  A *type II error* is the failure to
...
f <- function(n,mean=0){ t.test(rnorm(n,mean=mean),alternative="greater")$p.value}
par(mfrow=c(2,1))
hist(replicate(1000,f(n=10,mean=0)),col='pink')
hist(replicate(1000,f(n=10,mean=0.5)),col='pink')
```
