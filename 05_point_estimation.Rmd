...
arbitrary positive value $C$ and come to the same conclusions: it is
only _relative_ changes in likelihood that matter, not the absolute
value.  For the example of 35 successes out of 100 trials the
...
The combinatorical term ${100\choose 35}$ is just a constant, and such
constants can be very difficult to evaluate.  But we can choose the
constant $C$ to be any positive number, and if we choose
$C=\frac{1}{{100\choose 35}}$, then the combinatorial term in the
binomial cancels out and we are left with

\begin{equation}
\like{p}=\frac{1}{{100\choose 35}}{100\choose 35}p^{35}(1-p)^{65}=p^{35}(1-p)^{65}
\end{equation}
...
mean any function of $H$ that is proportional to $\prob{D|H}$.].  In R
it is just
...
We will see many examples where the arbitrary constant $C$ saves us a
huge amount of mathematical difficulty.  Later when we consider
Bayesian analysis we will see how this line of reasoning allows us to
pursue a different form of statistical inference.
