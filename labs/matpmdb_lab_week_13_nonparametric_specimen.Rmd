...
title: Nonparametric methods, suggested analysis
fontsize: 12pt
---
...
```{r}
plot(ecdf(a))
plot(ecdf(b),col='red',add=TRUE)
legend("topleft",c("black","red"),lty=1,pch=16,legend=c("a","b"))
```
...
**The two datasets differ in width rather than central tendency**
...
**It looks like instrument "b" has a wider uncertainty than "a"**
...
```{r}
x <- replicate(10000,t.test(rnorm(5),rnorm(5,mean=1))$p.value)
table(x<0.05)
```

**Power of test about 26%**
...
```{r}
x <- replicate(10000,t.test(rcauchy(5),rcauchy(5,location=1))$p.value)
table(x<0.05)
```

**Power of test about 4.5%**
...
**The t-test assumes normality.  It has very low power if the
  underlying distribution is Cauchy**
...
generates 10 normal observations and calculates their mean.  Call this
"estimator 1".  Then we can calculate estimator 1 a total of 30 times:
...
**We know from lectures that the mean will be distributed with
  variance $1/10$.  Numerically:**

```{r}
var(replicate(1e5,mean(rnorm(10))))  # compare theoretical value of 0.1
```
...
```{r}
var(replicate(1e5,median(rnorm(10)))) # compare theoretical value of 0.157
```

**(Theoretical analysis shows that the variance will be approximately
  $\pi/20=0.157$)**
...
**The mean is better: it has a lower variance (both are unbiased)**
...
```{r}
var(replicate(1e5,mean(rcauchy(10))))
var(replicate(1e5,median(rcauchy(10))))
```

**It looks like the mean has a crazily high variance.  Caveat emptor!**
...
**First define a `cat()` function to simulate the cat playing on the
  keyboard:**

```{r}
cat_rv <- function(swipes=8, keys=as.character(0:9)){
  as.numeric(paste(sample(keys,swipes,replace=TRUE),collapse=""))
}
cat_rv()
cat_rv()
```

**Now sample from a regular Gaussian distribution but replace one
  observation with the cat's pawprints:**

```{r}
rnormcat <- function(n,mean=0,sd=1, ...){
   out <- rnorm(n,mean,sd)
   out[sample(seq_along(out),1)] <- cat_rv(...)
   return(out)
}
options(scipen=1000)  # makes the display nicer
rnormcat(10)          # spot the effect of the cat
rnormcat(10)
qqnorm(rnormcat(10))
```

**Now investigate the difference between mean and median:**

```{r}
var(replicate(1e3,mean(rnormcat(10))))
var(replicate(1e3,mean(rnormcat(10))))
var(replicate(1e3,median(rnormcat(10))))
```

**The mean is strongly affected by the outlier, but the median is
  robust to this.  If you have a cat, use the median.  Bad kitty!**