...
\newcommand{\courseName}[0]{Statistical inference}
...
\item
A army office is interested in the horizontal distance from the target of a set of rifle shots, measured in centimetres.  It is known from previous studies that the standard deviation of the horizontal distance is 0.6\,cm. 
<<>>=
...
c = 2
critical <-  xseq[xseq>=c]
polygon(x=c(c, critical),y=c(0, dnorm((critical),mean=0,sd=1)),col='gray')
polygon(x=c(-critical, -c),y=c(dnorm(-(critical), mean=0,sd=1), 0),col='gray')
text(c(-1.5*c, 0, 1.5*c), rep(0.1,3), c(expression(paste(plain(Reject ), H[0], sep=" ")), 
                                         "Do not reject",
