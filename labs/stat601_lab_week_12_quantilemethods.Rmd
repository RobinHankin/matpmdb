...
You can get help on this dataset by typing `?precip` at the R prompt.
We will analyse this dataset.  We will start by a histogram:
...
Specifically, the dataset appears to be bimodal; but it is not clear
whether the bimodality is an artefact of the plotting techique, or
genuinely present in the data.  To plot this dataset in a more
objective way, we can just plot the points:
...
useful and interesting technique to assess normality.  First we will
produce the standard ecdf graph by hand:

```{r}
n <- 100
x <- sort(rnorm(n))
quant <- seq(from=1/(2*n),to=1-1/(2*n),len=n)
plot(x,quantile=quant)
```
...
plot(pnorm(x),quantile=quant)   # x-axis distorted by pnorm()
```
...
```{r}
plot(x,qnorm(quant))   # y-axis distorted with qnorm()
```
...
```{r}
qqnorm(precip)   # Function qqnorm() is an R builtin
abline(mean(precip),sd(precip))  
```

Above, see how the approximate straight line of the `qqnorm` plot
shows that the data is approximately Gaussian.  Of course, there is
some random noise present due to the random nature of the sample.
