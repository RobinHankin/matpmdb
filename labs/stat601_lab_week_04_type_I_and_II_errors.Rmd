...
Recall the central definitions:
...
We can now assess the power of the test.  Recall that the power of a test is $1-\beta$, where $\beta$ is the probability of failing to reject the null, when the null is false.  To calculate this, we need to specify a particular alternative.

We will start with $H_A\colon\mu=1$.  What is the probability of failing to reject the null now?

```{r}
replicate(100,mean(rnorm(n,mean=1))<0.75)
```

The null is incorrect here (`mean=1`) so we want to reject it.  If the sample mean is greater than 0.75 we correctly reject the null.  Alternatively, if the sample mean is less than 0.75 we fail to reject the null, and commit a type II error.  So in the above, the `TRUE` corresponds to a type II error.

We can use `table()` to summarize:


```{r}
table(replicate(1000,mean(rnorm(n,mean=1))<0.75))
```

So $\beta$ is about 0.26 and the power is about 0.74.
We could replace the 0.75 with the exact value:

```{r}
table(replicate(10000,mean(rnorm(n,mean=1))<qnorm(0.95,sd=1/sqrt(n))))
```

See how the test has a power of about 74\%.  This is the probability of correctly rejecting $H_0$ when it is false. We can calculate the value exactly:

```{r}
1-pnorm(qnorm(0.95,sd=1/sqrt(n)),mean=1,sd=1/sqrt(n))
pnorm(qnorm(0.95,sd=1/sqrt(n)),mean=1,sd=1/sqrt(n),lower.tail=FALSE)
```
(remember you can view the screencasts on `pnorm` etc to remind yourself what these functions do).
...
Vary the number of observations (choose three or four representative values in the range 5-50), and find the power of the test against for the same null, $H_0\colon\mu=0$ against different alternatives:  $H_A\colon\mu=0.1,0.2,0.5,1,2$.
...
The R language can accommodate the uncertainty in the standard deviation using the t.test() function.  Remember to consult `?t.test` for further information.

In practice you can just type `t.test()` on the terminal to test a particular null hypothesis:


```{r}
d <- c(2.1, 2.3, -0.1, 0.9, -1.08, 1.72, 0.59)
t.test(d,alternative="greater")
```

See how the p-value is less than 5% so we reject the null (which is $\mu=0$ by default).
...
(Here the null is incorrect so we want to reject it (that is, observe a p-value less than 0.05).  Obtaining a p-value greater than 0.05 corresponds to a type II error.  And $\beta$ is the probability of committing a type I error which in this caes is about 38%; the power is about 61%.
...
With such a tiny difference between the null and alternative hypotheses, we see that the power has declined to about 8%.


# Your task
 
Calculate the power of the t-test, varying the number of observations from n=5,10,20 and the alternative hypothesis 0.1,0.2,0.5,1.0,2.  I will collate.
